...
use windows::Win32::Graphics::Gdi::{CreateFontW, GetStockObject, CLEARTYPE_QUALITY,
                                   CLIP_DEFAULT_PRECIS, DEFAULT_CHARSET, DEFAULT_GUI_FONT, FF_DONTCARE,
                                   FW_NORMAL, HBRUSH, OUT_DEFAULT_PRECIS};
use windows::Win32::System::LibraryLoader::GetModuleHandleW;
use windows::Win32::UI::Controls::{InitCommonControlsEx, EM_REPLACESEL, EM_SETSEL,
                                  ICC_PROGRESS_CLASS, INITCOMMONCONTROLSEX,
...
fn control(class: PCWSTR, text: PCWSTR, style: u32, x: i32, y: i32, cx: i32, cy: i32,
    ...
    unsafe {
        let h = CreateWindowExW(
            ...
            dp(x), dp(y), dp(cx), dp(cy), Some(parent), Some(HMENU(id as *mut _)), None, None,
        ).unwrap_or_default();
        let font = FONT.load(Ordering::Relaxed);
        let font = if font == 0 { GetStockObject(DEFAULT_GUI_FONT).0 as usize } else { font as usize };
...
fn enable_actions(hwnd: HWND, on: bool) {
    for id in [ID_REFRESH, ID_SCAN, ID_IMAGE, ID_MOUNT, ID_UNMOUNT, ID_UNDELETE,
               ID_CARVE, ID_ERASE_INFO, ID_ERASE, ID_RESTORE] {
        if let Ok(h) = unsafe { GetDlgItem(Some(hwnd), id as i32) } {
            unsafe { let _ = EnableWindow(h, on); }
        }
    }
...
fn set_progress(bar: HWND, pct: Option<u32>) {
    unsafe {
        SendMessageW(bar, PBM_SETPOS, Some(WPARAM(pct.unwrap_or(0) as usize)), None);
    }
}
...
fn cancel(log: HWND) {
    match CHILD.lock().unwrap().as_mut() {
        Some(c) => {
            let _ = c.kill();
            append(log, "[!] cancelled. The child was killed, so its own cleanup did\r\n");
            append(log, "    not run: an image may be left attached and a VSS snapshot\r\n");
            append(log, "    left behind. A half-written image is not a usable backup.\r\n");
        }
...
"));
            ...
            match id {
                ...
                ID_RESTORE => {
                    if let (Some(t), true) = (need_target("restore onto"),
...
onto {t}?
                        ...
                        {
                            run(log, vec!["restore".into(), image_path.clone(), t],
                                Some("YES".into()));
                        }
                    }
                }
                ID_ERASE => {
                    let serial = unsafe { GetDlgItem(Some(hwnd), ID_CONFIRM as i32) }
                        .map(text_of).unwrap_or_default();
                    if let (Some(t), true) = (need_target("erase"), need(&serial,
...
Every sector is destroyed. There is no undo.
                ...
                ID_UNDELETE => {
                    ...
                    {
                        run(log, vec!["undelete".into(), t, "--to".into(), out_dir.clone()], None);
                    }
                }
                ID_CARVE => {
                    if let (Some(t), true) =
                    ...
                    {
                        run(log, vec!["carve".into(), t, "--to".into(), out_dir.clone()], None);
                    }
                }
                _ => {}
            }
        ...
        WM_SIZE => {
            ...
            if let Some((list, img, dir, log)) = snap {
                ...
                unsafe {
                    ...
                    let _ = MoveWindow(log, dp(10), dp(398), w - dp(20),
                                       (h - dp(408)).max(dp(40)), true);
                }
            }
            LRESULT(0)
        }
...
pub fn run_gui() -> Res<()> {
    unsafe {
        ...
        let f = CreateFontW(-dp(12), 0, 0, 0, FW_NORMAL.0 as i32, 0, 0, 0,
                            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                            CLEARTYPE_QUALITY, FF_DONTCARE.0 as u32,
                            w!("Segoe UI"));
        ...
        control(w!("STATIC"), w!("Drive serial (to erase):"), 0, 10, 253, 135, 16, hwnd, 0);
        control(w!("EDIT"), PCWSTR::null(), (WS_BORDER | WS_TABSTOP).0,
                150, 250, 200, 22, hwnd, ID_CONFIRM);
        control(w!("BUTTON"), w!("Overwrite every sector"),
                WS_TABSTOP.0 | BS_AUTOCHECKBOX as u32, 365, 252, 200, 20, hwnd, ID_OVERWRITE);
        ...
        for (id, label, width) in [
            ...
            control(w!("BUTTON"), label, WS_TABSTOP.0, x, 284, width, 26, hwnd, id);
            x += width + 6;
        }
        ...
        for (id, label, width) in [
            ...
            control(w!("BUTTON"), label, WS_TABSTOP.0, x, 316, width, 26, hwnd, id);
            x += width + 6;
        }

        let bar = control(w!("msctls_progress32"), PCWSTR::null(), 0,
                          10, 352, 740, 18, hwnd, ID_PROGRESS);
        SendMessageW(bar, PBM_SETRANGE32, Some(WPARAM(0)), Some(LPARAM(100)));
        control(w!("STATIC"), PCWSTR::null(), 0, 10, 376, 740, 16, hwnd, ID_STATUS);

        let log = control(w!("EDIT"), PCWSTR::null(),
                          (WS_BORDER | WS_VSCROLL).0
                              | ES_MULTILINE as u32 | ES_READONLY as u32 | ES_AUTOVSCROLL as u32,
                          10, 398, 740, 250, hwnd, ID_LOG);
        ...
        append(log, "  Erase info       what erase commands a drive supports\r\n");
        append(log, "  Erase disk       destroys it. Type the serial on the left first\r\n");
        append(log, "  Restore          writes an image back over a whole disk\r\n");
        append(log, "\r\nErase and Restore cannot be undone. Both ask again before they\r\n");
        append(log, "start, and Erase refuses unless the serial you typed matches the\r\n");
        append(log, "one the drive reports. part move and scan --rebuild stay on the\r\n");
